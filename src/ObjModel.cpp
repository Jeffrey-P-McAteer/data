...
void ObjModel::ReadFile(string fileName)
{
	...
	//for each (pair<string, Material> mtl in materials)
	//{
	//	cout << mtl.first << endl;
	//	cout << mtl.second.Ka[0] << ' ' << mtl.second.Ka[1] << ' ' << mtl.second.Ka[2] << endl;
	//	cout << mtl.second.Ns << endl;
	//}
	...
	for (int i = 0; i < mtlNames.size(); i++)
	{
		...
		if (mtl.d == 1)
			opaqueMaterials.push_back(mtlNames[i]);
		else
			translucentMaterials.push_back(mtlNames[i]);
	}
	...
	//cout << "# Face materials: " << facesInMaterials.size() << endl;
}

void ObjModel::DrawMaterials(string mtlName, bool opaque)
{
	...
	if (mtl.textureFileName != "")
	{
		...
		glBindTexture(GL_TEXTURE_2D, textures[mtlName]);
	...
	vector<int> faceIndices = facesInMaterials[mtlName];
